...
const CreateTask = () => {
  ...
  const getMentorManagers = async () => {
    const res = await axios.get(
      "https://mms-team-7.onrender.com/api/v1/mentor_managers",
      {
        headers: {
  ...
  const getMentor = async () => {
    const res = await axios.get(
      "https://mms-team-7.onrender.com/api/v1/mentors",
      {
        headers: {
          "Content-Type": "application/json",
          accept: "application/json",
            ...
            <div>
              {mentorManagerData.data?.map((manager: any, index: number) => (
                ...
                >
                  ...
                  <div>
                    {isMentorManagerInArray(manager.id) ? (
                      <Button onClick={() => removeMentorManager(manager.id)}>
                        <CheckCircle size={16} className="text-mmsPry3" />
                      </Button>
                    ) : (
                      <Button onClick={() => addMentorManager(manager.id)}>
            ...
            <div>
              {mentorData.data?.map((mentor: any, index: number) => (
                ...
                >
                  ...
                  <div>
                    {isMentorInArray(mentor.id) ? (
                      <Button onClick={() => removeMentor(mentor.id)}>
                        <CheckCircle size={16} className="text-mmsPry3" />
                      </Button>
                    ) : (
                      <Button onClick={() => addMentor(mentor.id)}>
                        <PlusCircle size={16} className="text-mmsPry3" />
                      </Button>
                    )}
                  </div>
